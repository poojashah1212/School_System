...
exports.createSession = async (req, res) => {
  try {
    ...
    if (overlap) {
      return res
        .status(400)
        .json({ message: "Session overlaps with existing session" });
    }
    ...
    if (!availability) {
      return res
        .status(400)
        .json({ message: "Teacher availability not set" });
    }

    if (!isWithinAvailability(availability, start, end)) {
      return res
        .status(400)
        .json({ message: "Session outside teacher availability" });
    }

    if (isHoliday(availability, start, end)) {
      return res
        .status(400)
        .json({ message: "Session falls on a holiday" });
    }
