const { body, validationResult } = require("express-validator"); const moment = require("moment");

exports.createSessionSlotsValidation = [
  body("title").trim().notEmpty().withMessage("Session title is required"), body("date").notEmpty().withMessage("date is required")
    .custom(value => {
      const parsed = moment(value, "DD-MM-YYYY", true); if (!parsed.isValid()) {
  ...
  body("student_id").optional().isMongoId().withMessage("student_id must be a valid.")
];
